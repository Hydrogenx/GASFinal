...
#include "AsyncCooldownListener.h"





UAsyncCooldownListener* UAsyncCooldownListener::ListenForCooldownChange(UAbilitySystemComponent* AbilitySystemComponent, FGameplayTagContainer InCooldownTags, bool InUseServerCooldown)
{
	UAsyncCooldownListener* ListenForCooldownChange = NewObject<UAsyncCooldownListener>();
	...
	AbilitySystemComponent->OnActiveGameplayEffectAddedDelegateToSelf.AddUObject(ListenForCooldownChange, &UAsyncCooldownListener::OnActiveGameplayEffectAddedCallback);
	...
	{
		AbilitySystemComponent->RegisterGameplayTagEvent(CooldownTag, EGameplayTagEventType::NewOrRemoved).AddUObject(ListenForCooldownChange, &UAsyncCooldownListener::CooldownTagChanged);
	}

	return ListenForCooldownChange;
}

void UAsyncCooldownListener::EndTask()
{
	if (IsValid(ASC))
	{
...
void UAsyncCooldownListener::OnActiveGameplayEffectAddedCallback(UAbilitySystemComponent* Target, const FGameplayEffectSpec& SpecApplied, FActiveGameplayEffectHandle ActiveHandle)
{
...
void UAsyncCooldownListener::CooldownTagChanged(const FGameplayTag CooldownTag, int32 NewCount)
{
	if (NewCount == 0)
	{
...
bool UAsyncCooldownListener::GetCooldownRemainingForTag(FGameplayTagContainer InCooldownTags, float& TimeRemaining, float& CooldownDuration)
{
